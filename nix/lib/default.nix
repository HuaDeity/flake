{inputs, ...}: {
  ...
  mkNixPackages = {
    ...
    systemFilteredPackages =
      lib.filterAttrs (
        name: attrs:
          if darwinMode
          then attrs ? systems && lib.elem pkgs.system attrs.systems
          else !attrs ? systems || lib.elem pkgs.system attrs.systems
      )
      packages;

    # Convert dot notation to nested attribute access
    getPackage = pkgPath: let
      parts =
        if builtins.isList pkgPath
        then pkgPath
        else lib.splitString "." pkgPath;
    in
    ...
    flakeList =
      lib.mapAttrsToList (
        name: _:
          inputs.${name}.packages.${pkgs.system}.default or null
      )
      flakePackages;
  in {
  ...
  mkBrewPackages = {
    ...
    packages =
      lib.filterAttrs (
        name: attrs: !attrs ? systems
      )
      originalPackages;

    normalizePath = pkgPath:
      if builtins.isList pkgPath
      then builtins.concatStringsSep "." pkgPath
      else pkgPath;
    ...
    converted = lib.mapAttrsToList (name: attrs: let
